...
static int handle_ra_pio(_unused struct ubus_context *ctx, _unused struct ubus_object *obj,
		...
		_unused struct blob_attr *msg)
{
	char ipv6_str[INET6_ADDRSTRLEN + 4];
	time_t now = odhcpd_time();
	struct interface *iface;
	...
	avl_for_each_element(&interfaces, iface, avl) {
		...
		for (size_t i = 0; i < iface->pio_cnt; i++) {
			...
			ipv6_strlen = strnlen(ipv6_str, sizeof(ipv6_str));
			snprintf(&ipv6_str[ipv6_strlen], sizeof(ipv6_str) - ipv6_strlen, "/%u", cur_pio->length);
