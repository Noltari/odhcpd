	...
	if (json) {
		...
		for (i = 0; i < json_object_array_length(interfaces_json); i++) {
			...
			for (j = 0; j < json_object_array_length(slaac_json); j++) {
				...
				const bool pio_inf = (lifetime == UINT32_MAX);
				uint32_t valid_lt;
				...
				if (valid_lt) {
					...
					if (iface->pio) {
						...
						uint8_t pio_len = (uint8_t) json_object_get_uint64(length_json);
						const char *pio_str = json_object_get_string(prefix_json);
						struct ra_pio *pio = &iface->pio[iface->pio_cnt - 1];
		...
		memcpy(ra_pio_md5, new_md5, sizeof(ra_pio_md5));
	}
}
...
static void _config_save_ra_piofile(void)
{
	...
	avl_for_each_element(&interfaces, iface, avl) {
		...
		for (i = 0; i < iface->pio_cnt; i++) {
			...
			md5_hash(cur_pio, sizeof(*cur_pio), &md5);
	...
	if (memcmp(ra_pio_md5, new_md5, sizeof(ra_pio_md5))) {
